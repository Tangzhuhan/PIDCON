...
class EnlargedPlotWindow(QMainWindow):
    def __init__(self, plot_widget, title):
            ...
            if isinstance(item, pg.PlotDataItem):
                try:
                    ...
                    if x_data is not None and y_data is not None:
                        ...
                        self.curve_data[name] = {'x': x_data, 'y': y_data}
                except Exception as e:
        ...
        # 设置鼠标追踪阈值（像素）
        self.mouse_threshold = 10
        
        # 确保视图框已创建
        self.plot.plotItem.getViewBox()
    ...
    def mouse_moved(self, pos):
        ...
        if self.plot.sceneBoundingRect().contains(pos):
            ...
            for name, data in self.curve_data.items():
                ...
                for i in range(len(x_data)):
                    if not np.isnan(x_data[i]) and not np.isnan(y_data[i]):
                        # 将数据点转换为场景坐标
                        data_point = pg.Point(x_data[i], y_data[i])
                        scene_point = self.plot.plotItem.mapDataToScene(data_point)
                        # 计算与鼠标的像素距离
                        distance = ((scene_point.x() - pos.x()) ** 2 + 
                                  (scene_point.y() - pos.y()) ** 2) ** 0.5
                        
                        if distance < closest_distance and distance < self.mouse_threshold:
            ...
            if closest_point is not None:
                ...
                # 根据图表类型格式化显示
                if "Temperature" in self.windowTitle():
                    text = f"{closest_curve}\nTime: {x_val:.1f}s\nTemp: {y_val:.2f}°C"
                elif "Voltage" in self.windowTitle():
                    text = f"Time: {x_val:.1f}s\nVoltage: {y_val:.2f}V"
                elif "Current" in self.windowTitle():
                    text = f"Time: {x_val:.1f}s\nCurrent: {y_val:.3f}A"
                else:
                    text = f"{closest_curve}\nX: {x_val:.2f}\nY: {y_val:.2f}"
                
                # 更新提示位置和文本
                self.tip.setPos(x_val, y_val)
                self.tip.setHtml(f'<div style="text-align: center">{text}</div>')
                self.tip.show()
            else:
                self.tip.hide()
        else:
...
class MaterialParamsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setGeometry(200, 200, 400, 450)  # 增加窗口高度以容纳新控件
        
        layout = QVBoxLayout()
        
        # 材料名称输入
        self.material_name = QLineEdit()
        self.material_name.setPlaceholderText("Enter material name")
        layout.addWidget(QLabel("Material Name:"))
        layout.addWidget(self.material_name)
        
        # 目标温度输入
        self.target_temp = QDoubleSpinBox()
        self.target_temp.setRange(0, 999)  # 三位整数
        self.target_temp.setDecimals(2)  # 两位小数
        self.target_temp.setSingleStep(0.1)
        self.target_temp.setValue(25.0)  # 默认值
        layout.addWidget(QLabel("Target Temperature (°C):"))
        layout.addWidget(self.target_temp)
        
        # 参数输入
        self.initial_voltage = QDoubleSpinBox()
        self.initial_voltage.setRange(0, 30)
        self.initial_voltage.setDecimals(2)
        ...
        self.max_voltage.setRange(0, 30)
        self.max_voltage.setDecimals(2)
        self.max_voltage.setSingleStep(0.1)
        self.max_voltage.setValue(17.0)
        layout.addWidget(QLabel("Maximum Voltage (V):"))
        layout.addWidget(self.max_voltage)
        
        # 预热时间设置
        self.warmup_time = QDoubleSpinBox()
        self.warmup_time.setRange(0, 3600)  # 0-3600秒（1小时）
        self.warmup_time.setDecimals(0)  # 整数秒
        self.warmup_time.setSingleStep(1)
        self.warmup_time.setValue(30)
        layout.addWidget(QLabel("Warmup Time (s):"))
        layout.addWidget(self.warmup_time)
        
        # PID参数设置 - 使用QDoubleSpinBox并增加小数位数
        self.kp = QDoubleSpinBox()
        self.kp.setRange(0, 100)
        self.kp.setDecimals(3)  # 三位小数
        self.kp.setSingleStep(0.001)
        self.kp.setValue(0.200)
        layout.addWidget(QLabel("Kp:"))
        layout.addWidget(self.kp)
        
        self.ki = QDoubleSpinBox()
        self.ki.setRange(0, 100)
        self.ki.setDecimals(3)  # 三位小数
        self.ki.setSingleStep(0.001)
        self.ki.setValue(0.002)
        layout.addWidget(QLabel("Ki:"))
        layout.addWidget(self.ki)
        
        self.kd = QDoubleSpinBox()
        self.kd.setRange(0, 100)
        self.kd.setDecimals(3)  # 三位小数
        self.kd.setSingleStep(0.001)
        self.kd.setValue(0.005)
    ...
    def set_params(self, params):
        """设置所有参数"""
        self.material_name.setText(params.get('material_name', ''))
        self.target_temp.setValue(params.get('target_temp', 25.0))
        self.initial_voltage.setValue(params.get('initial_voltage', 17.0))
        self.max_voltage.setValue(params.get('max_voltage', 30.0))
        self.warmup_time.setValue(params.get('warmup_time', 30))
        self.kp.setValue(params.get('kp', 0.200))
        self.ki.setValue(params.get('ki', 0.002))
        self.kd.setValue(params.get('kd', 0.005))
    ...
    def save_params(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getSaveFileName(
                ...
                os.path.join(os.path.expanduser("~"), f"{params['material_name']}_params.json"),
                "JSON Files (*.json)"
            )
    ...
    def load_material_params(self):
        ...
        try:
            # 获取配置目录
            config_dir = user_data_dir('PIDTempControl', 'Personal')
            config_file = os.path.join(config_dir, 'material_params.json')
            ...
            print(f"Failed to load default parameters: {e}")
...
class PIDSystemUI(QMainWindow):
    ...
    def add_left_panel_components(self):
        ...
        self.sampling_rate_input.setText("30")
        ...
        self.max_voltage_input.setText("17.0")
    ...
    def start_control(self):
        ...
        try:
            ...
            warmup_time = float(self.warmup_time_input.text())
            duration = float(self.duration_input.text())
            temp_error = float(self.tolerance_input.text())
            
            # 参数验证
            if max_voltage <= 0 or max_voltage > 17:
                QMessageBox.warning(self, "警告", "最大电压必须在0-17V之间")
                return
    ...
    def save_material_params(self, dialog):
        ...
        try:
            ...
            self.warmup_time_input.setText(str(params['warmup_time']))
            self.kp_input.setText(str(params['kp']))
            self.ki_input.setText(str(params['ki']))
            self.kd_input.setText(str(params['kd']))
            
            QMessageBox.information(self, "Success", "Material parameters saved successfully!")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to save material parameters: {str(e)}")
    ...
    def handle_legend_click(self, evt):
        ...
        if evt.button() == Qt.LeftButton:
            ...
            if legend and legend.sceneBoundingRect().contains(pos):
                # 获取点击的图例项
                for item in legend.items:
                    if item.sceneBoundingRect().contains(pos):
                        # 切换曲线的可见性
                        curve_name = item.text
                        for curve in plot.plotItem.items:
                            if isinstance(curve, pg.PlotDataItem) and curve.name() == curve_name:
                                curve.setVisible(not curve.isVisible())
                                # 更新图例项的可见性
                                item.setVisible(curve.isVisible())
                                break
                        break
    ...
    def load_material_params(self):
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                # 更新主界面的参数
                if params:
                    self.setpoint_input.setText(str(params.get('target_temp', '')))
                    self.initial_voltage_input.setText(str(params.get('initial_voltage', '')))
                    self.max_voltage_input.setText(str(params.get('max_voltage', '')))
                    self.warmup_time_input.setText(str(params.get('warmup_time', '')))
                    self.kp_input.setText(str(params.get('kp', '')))
                    self.ki_input.setText(str(params.get('ki', '')))
                    self.kd_input.setText(str(params.get('kd', '')))
                print("已加载保存的材料参数")
            else:
                print("未找到保存的材料参数文件")
        except Exception as e:
